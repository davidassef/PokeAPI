...
def test_translation_system():
    ...
    # Teste 2: Verificar traduções para Pokémon especificado
    pokemon_id = int(sys.argv[1]) if len(sys.argv) > 1 else 1
    pokemon_names = {1: "Bulbasaur", 2: "Ivysaur", 3: "Venusaur", 4: "Charmander"}
    pokemon_name = pokemon_names.get(pokemon_id, f"Pokémon #{pokemon_id}")
    print(f"\nTestando traduções para Pokémon {pokemon_id} ({pokemon_name}):")
